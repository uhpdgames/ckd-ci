<?php
    $iskhuyenmai = !empty($_REQUEST['khuyenmai']) ? htmlspecialchars($_REQUEST['khuyenmai']) : "0";
	...
	$linkMan = "index.php?com=product&act=man&type=".$type."&p=".$curPage."&khuyenmai=".$iskhuyenmai;
	if($act=='add') $linkFilter = "index.php?com=product&act=add&type=".$type."&p=".$curPage."&khuyenmai=".$iskhuyenmai;
	else if($act=='edit') $linkFilter = "index.php?com=product&act=edit&type=".$type."&p=".$curPage."&id=".$id."&khuyenmai=".$iskhuyenmai;
    if($act=="copy") $linkSave = "index.php?com=product&act=save_copy&type=".$type."&p=".$curPage."&khuyenmai=".$iskhuyenmai;
    else $linkSave = "index.php?com=product&act=save&type=".$type."&p=".$curPage."&khuyenmai=".$iskhuyenmai;
                                    ...
                                    <?php foreach($config['website']['lang'] as $k => $v) { ?>
                                        <div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
                                            <div class="form-group">
                                                ...
                                                <input onkeypress ="updateSEO()" type="text" class="form-control for-seo" name="data[ten<?=$k?>]" id="ten<?=$k?>" placeholder="Tiêu đề (<?=$k?>)" value="<?=@$item['ten'.$k]?>" <?=($k=='vi')?'required':''?>>
                                            </div>
                                            ...
                                            <?php } ?>
                                            <div class="form-group">
                                                <label for="noidungthanhphan<?=$k?>">Nội dung thành phần (<?=$k?>):</label>
                                                <textarea class="form-control form-control-ckeditor" name="data[noidungthanhphan<?=$k?>]" id="noidungthanhphan<?=$k?>" rows="5" placeholder="Nội dung (<?=$k?>)"><?=htmlspecialchars_decode(@$item['noidungthanhphan'.$k])?></textarea>
                                            </div>
                                        </div>
                                    <?php } ?>
                                </div>
                        ...
                        <div class="card card-primary card-outline text-sm">
                            ...
                            <div class="card-body">
                                <div class="form-group-category row">
                                    ...
                                    <div class="form-group col-xl-6 col-sm-4">
                                        <label class="d-block" for="id_thuonghieu">Thương hiệu:</label>
                                        <?=$func->get_them('news', 'thuong-hieu','id_thuonghieu', 'Chọn thương hiệu')?>
                                    </div>

                                    <div class="form-group col-xl-6 col-sm-4">
                                        <label class="d-block" for="id_dong">DÒng:</label>
                                        <?=$func->get_them('news', 'dong','id_dong', 'Chọn dòng')?>
                                    </div>
                       ...
                       <div class="card card-primary card-outline text-sm">
                           ...
                           <div class="card-body">
                               ...
                               <div class="row">
                                   ...
                                   <?php if(isset($config['product'][$type]['khoiluong']) && $config['product'][$type]['khoiluong'] == true) { ?>
                                       <div class="form-group col-md-4">
                                           <label class="d-block" for="khoiluong">Khối lượng :</label>
                                           <div class="input-group">
                                               <input type="text" class="form-control gia_km" name="data[khoiluong]" id="khoiluong" placeholder="Khối lượng " value="<?=@$item['khoiluong']?>" maxlength="3" >
                                               <div class="input-group-append">
                                   ...
                                   <div class="form-group col-md-4">
                                       <label class="d-block" for="thetich">Thể tích:</label>
                                       <div class="input-group">
                                       <input type="text" class="form-control" name="data[thetich]" id="thetich" placeholder="Thể tích" value="<?=@$item['thetich']?>">
                                       <!--<div class="input-group-append">
                               ...
                               <div class="row">
                                   <div class="col-4">
                                       <div class="form-group">
                                           <label class="d-block" for="xxx" >Số lượng đã bán (số thực): <span><?=@$item['daban'];?></span></label>
                                       </div>
                                   </div>
                                   <div class="form-group col-md-4">
                                       <label class="d-inline" for="daban"> Số lượng đã bán:</label>
                                       <input type="text" class="form-control" name="data[nhaplieu_daban]" id="daban" placeholder="Nhập liệu số lượng sẽ hiển thị trên website" value="<?=@$item['nhaplieu_daban']?>">
                                       <small>(hiển thị trên website = số thực + số nhập liệu)</small>
                                   </div>

                                   <?php if($iskhuyenmai):?>
                                       <div class="form-group col-md-4">
                                           <label class="d-block" for="daban"> Ngày hết hạn khuyến mãi:</label>
                                           <input type="text" class="form-control" name="data[khuyenmai_ngayhethan]" id="khuyenmai_ngayhethan" value="<?=@$item['khuyenmai_ngayhethan']?>">
                                       </div>
                                   <?php endif;?>
                               </div>
                           </div>
                        <!--   <div class="card-footer">
                               <span class="badge border-danger border-1 text-danger">
                                   ...
                                   <strong>Vui lòng không nhập <span class="border-warning border-1 text-primary">đơn vị tính</span> vào ô nhập liệu!</strong>
                               </span>
                           </div>-->
                       </div>
...
/*                    }
        ...
        <?php if(isset($config['product'][$type]['seo']) && $config['product'][$type]['seo'] == true) { ?>
			<div class=" card-outline text-sm">
	            <div class="card-header">
        ...
        <div class="card-footer text-sm">
            ...
            <input type="hidden" name="data[khuyenmai]" value="<?=$iskhuyenmai;?>">
        </div>
    </form>
</section>
...
<script>
	...
	function updateSEO(){
		let title = $('#tenvi').val();
		if(title !='') $('#titlevi').val(title);


		//var url = removeAccents(title);
		//console.log(url);

		slugPreview(title, 'vi' );
		//slugvi(url);
    ...
    $(document).ready(function(){
        $('#khuyenmai_ngayhethan').datetimepicker({
            timepicker: false,
            format: 'd/m/Y H:m:s',
    ...
    function updatedelete(){
        $('.tr-delete').on('click', function(event) {
