        ...
        <div class="card card-primary card-outline text-sm">
            ...
            <div class="card-body row">
                ...
                <div class="form-group text-center mt-2 mb-0 col-12">
                    <a class="btn btn-success rounded-pill text-white" onclick="actionOrder('<?=$linkFilter?>')" title="Tìm kiếm"><i class="fas fa-search mr-1"></i>Tìm kiếm</a>
                    <a class="btn btn-danger rounded-pill ml-1" href="<?=$linkMan?>" title="Hủy lọc"><i class="fas fa-times mr-1"></i>Hủy lọc</a>
                </div>
            </div>
        </div>
    <?php } ?>
    <div class="card card-primary card-outline text-sm mb-0">
        <div class="card-header">
            ...
            <?php if(isset($config['order']['excelall']) && $config['order']['excelall'] == true) { ?>
                <a class="btn btn-success rounded-pill btn-export-excel btn-sm d-inline-block align-middle ml-2 text-white" onclick="actionOrder('<?=$linkExcel?>')" title="Xuất file Excel"><i class="far fa-file-excel mr-1"></i>Xuất file Excel</a>
            <?php } ?>
            <?php if(isset($config['order']['wordall']) && $config['order']['wordall'] == true) { ?>
                <a class="btn btn-warning rounded-pill btn-export-word btn-sm d-inline-block align-middle ml-2 text-white" onclick="actionOrder('<?=$linkWord?>')" title="Xuất file Word"><i class="far fa-file-word mr-1"></i>Xuất file Word</a>
            <?php } ?>
        </div>
                        ...
                        <?php for($i=0;$i<count($items);$i++) { ?>
                            <tr>
                                ...
                                <td class="align-middle text-center text-md text-nowrap">
                                    ...
                                    <a class="text-success mr-2" onclick="PrintBild('<?=$items[$i]['id_tracking']?>');" href="javascript:void(0)" title="Print Bill"><i class="fas fa-print"></i></a>    
                                    <?php }?>
...
<script type="text/javascript">
    function PrintBild(id_order){
        $.ajax({
            url: 'ajax/printbill.php',
            type: 'POST',
            dataType: 'json',
            data: {id_order: id_order},
            success:function(data){
                if(data.status == 200 && data.error == false){
                    let link ='https://digitalize.viettelpost.vn/DigitalizePrint/report.do?type=2&bill='+data.message+'&showPostage=1';
