                    ...
                    <?php foreach($config['website']['slug'] as $k => $v) { ?>
                        <div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-sluglang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
                            <div class="form-gourp mb-0">
                                <label class="d-block">Đường dẫn mẫu (<?=$k?>):<a href="<?=$config_base?><?=@$item['tenkhongdau'.$k]?>" target="_blank"><span class="pl-2 font-weight-normal" id="slugurlpreview<?=$k?>"><strong class="text-info"><?=$config_base?><?=@$item['tenkhongdau'.$k]?></strong></span></a></label>
