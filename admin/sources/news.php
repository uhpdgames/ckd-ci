<?php
	...
	function save_item()
	{
		...
		if($id && $act!='save_copy')
		{
			...
			{
				$file_nameicon = $func->uploadName($_FILES['icon']["name"]);
				if($icon = $func->uploadImage("icon", $config['news'][$type]['img_type'], UPLOAD_NEWS,$file_nameicon))
				{
					$data['icon'] = $icon;
					$row = $d->rawQueryOne("select id, icon from #_news where id = ? and type = ? limit 0,1",array($id,$type));
					if(isset($row['id']) && $row['id'] > 0) $func->delete_file(UPLOAD_NEWS.$row['icon']);
				}
			}
