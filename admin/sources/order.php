...
function save_item()
{
    ...
    if ($id) {
        if ($ref_uid != 0) {
            $_Affiliate->setUserUid($ref_uid);

            $aff_data = $wallet_data = array();
            $aff_data['code'] = (isset($_POST['ref_code'])) ? htmlspecialchars($_POST['ref_code']) : 0;

            // $wallet_data['tempBalance'] = $id;
            $wallet_data['orderId'] = $id;
            $wallet_data['code'] = $aff_data['code'];
            $wallet_data['uid'] = $ref_uid;
            $wallet_data['tinhtrang'] = $data['tinhtrang'];
            ...
            if (isset($data['tinhtrang'])) {
                ...
                if ($data['tinhtrang'] == 5 || $data['tinhtrang'] == 4) {
                    $aff_data['hienthi'] = 1;
                }

                if ($_Affiliate->_insertRefOrder($aff_data, $id)) {

                    $level = $_Affiliate->upgradeLevel();

                    if ($level > 0) {
                        $_Affiliate->insertWallet($wallet_data);
                    }
                }
            }
        }
