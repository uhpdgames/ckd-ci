<?php
	...
	switch($act)
	{
        ...
        /*case "promotion":
            get_items();
            $template = "product/promotion/items";
            break;*/
	...
	function get_items()
	{
        ...
        $is_khuyenmai = (isset($_REQUEST['khuyenmai'])) ? htmlspecialchars($_REQUEST['khuyenmai']) : false;
		...
		if(isset($_REQUEST['keyword']))
		{
			...
			$where .= " and (tenvi LIKE '%$keyword%' or tenen LIKE '%$keyword%' or masp LIKE '%$keyword%')";
		}
		...
		$sql = "select * from #_product where `delete` = 0 and type = ? $where order by hienthi desc, stt asc $limit";
...
/*        if($gift){
            foreach($gift as $column => $value)
            {
                ...
                {
                    foreach($value as $k2 => $v2) $option2[$k2] = $v2;
                    $gift[$column] = json_encode($option2);
                }
                else
                {
                    $gift[$column] = htmlspecialchars($value);
                }
            }
            $gift['product_id'] = 'product_id';
            $d->insert('product_gift', $gift);
        }*/
	...
	function save_list()
	{
		...
		if($id)
		{
			...
			{
				$file_name_icon = $func->uploadName($_FILES['file']["icon"]);
