		...
		<?php if (is_array($order) && count($order)) {
			foreach ($order as $k => $v) { ?>
				...
				<div class="item_dh">
					...
					<p class="phiship tongtien" style="color:indianred">Phí ship <?=format_money($order['phiship']) ??''?></p>
					<p class="tongtien" style="color:palevioletred">Giá trị đơn hàng: <?=format_money($order['tonggia']) ??''?></p>
				</div>
			<?php }
		} ?>
