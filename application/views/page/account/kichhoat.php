	...
	<div class="wrap-user">
		...
		<form class="form-user validation-user" novalidate method="post" action="account/kich-hoat?id=<?=$_GET['id']?>" enctype="multipart/form-data">
			<div class="input-group input-user">
				...
				<input type="text" class="form-control" id="maxacnhan" name="maxacnhan" placeholder="<?=getLang('nhapmakichhoat')?>" required>
				<div class="invalid-feedback"><?=getLang('vuilongnhapmakichhoat')?></div>
			</div>
			<div class="button-user">
				<input type="submit" class="btn btn-primary" name="kichhoat" value="<?=getLang('kichhoat')?>" disabled>
			</div>
		</form>
	</div>
