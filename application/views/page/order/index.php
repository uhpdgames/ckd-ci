				...
				<div class="col-12 col-lg-6 bottom-cart">
					...
					<div class="section-cart">
						...
						<div class="information-cart">
							...
							foreach ($httt as $key => $value) {
								...
								<div class="payments-cart custom-control custom-radio">
									...
									<label class="payments-label custom-control-label"
										   for="payments-<?= $value['id'] ?>"
									...
									<div
										class="payments-info payments-info-<?= $value['id'] ?> transition"><?= htmlspecialchars_decode($value['mota' . $lang] ?? "") ?></div>
								</div>
							<?php } ?>
						</div>
							...
							<div class="input-double-cart clear">
								...
								<div class="input-cart">
									<input type="number" onKeyPress="if(this.value.length==11) return false;"
										   ...
										   name="dienthoai"
										   placeholder="<?= getLang('sodienthoai') ?>"
										   value="<?= (isset($_SESSION[$login_member]['dienthoai'])) ? $_SESSION[$login_member]['dienthoai'] : '' ?>"
										   required/>
									<div class="invalid-feedback"
										 id="invalid-phone"><?= getLang('vuilongnhapsodienthoai') ?></div>
								</div>
							</div>
							<div class="input-cart">
								<input required type="email" class="form-control" id="email" name="email"
									   placeholder="Email"
									   value="<?= (isset($_SESSION[$login_member]['email'])) ? $_SESSION[$login_member]['email'] : '' ?>"/>
							</div>
						...
						<input type="submit" class="btn-cart btn btn-primary btn-lg btn-block" name="thanhtoan"
							   id="thanhtoan"
							   value="<?= getLang('thanhtoan') ?>" disabled>
					</div>
				</div>
			<?php } else { ?>
