<style>
    #collapseOne {
    overflow-y: auto;
    max-height: 50rem; /* Hoặc bất kỳ chiều cao cố định nào bạn muốn */
}
</style>
<div class="loc pc">
            <div id="accordion">
                <div class="card"
				style="border: none"
				>
                    <div class="card-header font-color-fitter" style="background-color: #fff !important;" id="headingOne" class="btn " data-toggle="collapse" data-target="#collapseOne" aria-expanded="true" aria-controls="collapseOne">
                                Bộ lọc sản phẩm
                    </div>
					
                    <div id="collapseOne" class="collapse show" aria-labelledby="headingOne" data-parent="#accordion">
                        <div class="card-body font-size-fitter">
						<div class="form-group row cover-row-fitter">
                                <div class="font-color-fitter-1">Nổi bật</div>
                                <div class="col-sm-10">
                                    <div class="form-check form-check-inline cover-font-radio">
                                        <input class="form-check-input" type="radio" name="New" id="inlineRadio1" value="option1" />
                                        <label class="form-check-label  font-lable-fitter" for="inlineRadio1">Tất cả</label>
                                    </div>                                    
                                </div>
                                <div class="col-sm-10">
                                    <div class="form-check form-check-inline cover-font-radio">
                                        <input class="form-check-input" type="radio" name="New" id="inlineRadio1" value="option3" />
                                        <label class="form-check-label  font-lable-fitter" for="inlineRadio1">Mới nhất</label>
                                    </div>                                    
                                </div>
                                <div class="col-sm-10">
                                    <div class="form-check form-check-inline cover-font-radio">
                                        <input class="form-check-input" type="radio" name="New" id="inlineRadio1" value="option2" />
                                        <label class="form-check-label  font-lable-fitter" for="inlineRadio1">Bán chạy</label>
                                    </div>                                    
                                </div>
                                
                            </div>
							
                            <div class="form-group row cover-row-fitter">
                                <div class="font-color-fitter-1">Loại sản phẩm</div>
                                <div class="col-sm-10">
                                    <div class="form-check form-check-inline cover-font-radio">
                                        <input class="form-check-input" type="radio" name="ProductType" id="inlineRadio1" value="option1" />
                                        <label class="form-check-label  font-lable-fitter" for="inlineRadio1">Làm sạch</label>
                                    </div>
                                    <div class="form-check form-check-inline cover-font-radio">
                                        <input class="form-check-input" type="radio" name="ProductType" id="inlineRadio2" value="option2" />
                                        <label class="form-check-label font-lable-fitter" for="inlineRadio2">Chăm sóc da</label>
                                    </div>
                                    <div class="form-check form-check-inline cover-font-radio">
                                        <input class="form-check-input" type="radio" name="ProductType" id="inlineRadio3" value="option3" />
                                        <label class="form-check-label font-lable-fitter" for="inlineRadio3">Mặt nạ</label>
                                    </div>
									<div class="form-check form-check-inline cover-font-radio">
                                        <input class="form-check-input" type="radio" name="ProductType" id="inlineRadio3" value="option3" />
                                        <label class="form-check-label font-lable-fitter" for="inlineRadio3">Chống nắng</label>
                                    </div>
                                </div>
                            </div>

							<div class="form-group row cover-row-fitter">
								<div class="form-group">
  									<label class="font-color-fitter-1" for="priceRange">Chọn Mức Giá:</label>
  										<select class="form-control" id="priceRange">
											 <option value="0">Tất cả</option>
   											 <option value="1-1000">Dưới 500,000</option>
   											 <option value="1000-5000">1tr đến 2tr</option>
   											 <option value="5000-10000">2tr đến 3tr</option>
   											 <option value="10000-20000">3tr đến 4tr</option>
    										 <option value="20000+">Trên 4tr</option>
  										</select>
								</div>
                        </div>
							<div class="form-group row cover-row-fitter">
                                <div class="font-color-fitter-1">Thương hiệu</div>
                                <div class="col-sm-10">
                                    <div class="form-check form-check-inline cover-font-radio">
                                        <input class="form-check-input" type="radio" name="Brand" id="brand" value="option1" />
                                        <label class="form-check-label font-lable-fitter" for="inlineRadio4">CKD</label>
                                    </div>
                                    <div class="form-check form-check-inline cover-font-radio">
                                        <input class="form-check-input" type="radio" name="Brand" id="brand" value="option2" />
                                        <label class="form-check-label font-lable-fitter" for="inlineRadio5">Bellasoo</label>
                                    </div>
                                    <div class="form-check form-check-inline cover-font-radio">
                                        <input class="form-check-input" type="radio" name="Brand" id="brand" value="option3" />
                                        <label class="form-check-label font-lable-fitter" for="inlineRadio6">Lactor-Derm</label>
                                    </div>
                                </div>
                            </div>
							<div class="form-group row cover-row-fitter">
                                <div class="font-color-fitter-1">Theo dòng CKD</div>
                                <div class="col-sm-10">
                                    <div class="form-check form-check-inline cover-font-radio">
                                        <input class="form-check-input" type="radio" name="inlineRadioOptions" id="inlineRadio1" value="option1" />
                                        <label class="form-check-label font-lable-fitter" for="inlineRadio1">Retinol-Collagen</label>
                                    </div>
                                    <div class="form-check form-check-inline cover-font-radio">
                                        <input class="form-check-input" type="radio" name="inlineRadioOptions" id="inlineRadio2" value="option2" />
                                        <label class="form-check-label font-lable-fitter" for="inlineRadio2">Vita C Teca</label>
                                    </div>
                                    <div class="form-check form-check-inline cover-font-radio">
                                        <input class="form-check-input" type="radio" name="inlineRadioOptions" id="inlineRadio3" value="option3" />
                                        <label class="form-check-label font-lable-fitter" for="inlineRadio3">Amino Biotin</label>
                                    </div>
									<div class="form-check form-check-inline cover-font-radio">
                                        <input class="form-check-input" type="radio" name="inlineRadioOptions" id="inlineRadio3" value="option3" />
                                        <label class="form-check-label font-lable-fitter" for="inlineRadio3">Keo ong xanh</label>
                                    </div>
                                </div>
                            </div>
							
                        </div>
                    </div>
                </div>
            </div>
        </div>