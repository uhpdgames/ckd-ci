...
//		$this->session->set_userdata('isLogin', $this->isLogin);
	...
	function login()
	{
		...
		$row = $this->d->rawQueryOne("select ngaysinh,gioitinh,ref_nick, id, password, username, dienthoai, diachi, email, ten from #_member where username = ? and hienthi > 0 limit 0,1", array($username));
		if (isset($row['id']) && $row['id'] > 0) {
			if ($row['password'] == $passwordMD5) {
				...
				$voucher = $this->d->rawQueryOne("select id as id_voucher,code as magiamgia,start_date,end_date,discount_amount,discount_percentage,is_one_time_use,is_combinable,used_date from #_coupons where type = ? and uid = ? and deleted = 0 limit 0,1", array('register', $row['id']));
	...
	function info_user()
	{
        ...
        if(empty($iduser)) redirect(site_url());
