<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#taovandon').click(function(event) {
		//	holdonOpen();
			$.ajax({
		...
		$("#form-ajax").submit(function (event) {
			...
			posting.done(function (data) {
				$('#results').fadeIn(500);
				$('#results').text('Cập nhật thành công!');
				setTimeout(function (){
					$('#results').fadeOut(500);
				}, 3000)
			});
			posting.fail(function () {
				$('#results').fadeIn(500);
				$('#results').text('Cập nhật thất bại');
				setTimeout(function (){
					$('#results').fadeOut(500);
				}, 3000)
			});
	...
	function refreshFilerIfEmpty()
    {
		...
		$.ajax({
			...
			success: function(result)
			{
				...
				+'<a class="btn btn-warning rounded-pill text-white check-all-filer mr-1"><i class="far fa-square mr-2"></i>Chọn tất cả</a>'
	...
	function notifyDialog(text)
	{
		const swalconst = Swal.mixin({
			customClass: {
				confirmButton: 'btn btn-warning rounded-pill text-sm',
			},
	...
	function confirmDialog(action,text,value)
	{
		const swalconst = Swal.mixin({
			customClass: {
				confirmButton: 'btn btn-warning rounded-pill text-sm mr-2',
				cancelButton: 'btn btn-danger rounded-pill text-sm'
			},
			buttonsStyling: false
		})
		swalconst.fire({
	...
	$(document).ready(function(){
		...
		$(".watermark-position label").click(function(){
			if($(".change-photo img").length)
			{
				...
				if(img)
				{
					$(".watermark-position label img").attr("src","assets/images/noimage.webp");
		...
		$("form#form-ajax").append('' +
			'' +
			'<input type="hidden" name="table" value="'+table_name+'" />'+
			'<input type="hidden" name="id" value="'+id_details+'" />'
		...
		$("#filer-gallery").filer({
            limit: null,
            maxSize: null,
            extensions: ["jpg","png","jpeg","JPG","PNG","JPEG","Png", "gif", "webp"],
		...
		$("#filer-import").filer({
            ...
            changeInput: '<a class="jFiler-input-choose-btn border-primary btn btn-warning rounded-pill text-white mb-3"><i class="fas fa-cloud-upload-alt mr-2"></i>Upload hình ảnh</a>',
			theme: "default",
            showThumbs: true,
            addMore: true,
            allowDuplicates: false,
