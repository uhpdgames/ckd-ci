...
(function ($, window) {
  ...
  FreezeTable.prototype.buildColumnTable = function() {
    ...
    this.$container.on('resize.'+this.namespace, function() {
      ...
      for (var i = 1; i <= columnNum; i++) {
        // th/td detection
        var th = that.$table.find('tr:last th:nth-child('+i+')').outerWidth();
  ...
  FreezeTable.prototype.simplifyHead = function (table) {
    ...
    $headTable.find("> tr, > tbody > tr, tfoot > tr").not(':last').remove();
    // Each th/td width synchronism
    $.each($headTable.find("> thead > tr:nth-child(2) >"), function (key, value) {
      
      var width = that.$table.find("> thead > tr:nth-child(2) > :nth-child("+parseInt(key+1)+")").outerWidth();
